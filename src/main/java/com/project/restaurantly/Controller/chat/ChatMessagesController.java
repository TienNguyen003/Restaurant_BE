...
//    @PreAuthorize("@requiredPermission.checkPermission('PERM_VIEW')")
    ...
    @GetMapping("/by")
    ApiResponse<List<ChatMessagesResponse>> getMenu(@RequestParam int status, @RequestParam long chatId) {
        return ApiResponse.<List<ChatMessagesResponse>>builder()
    ...
    @PutMapping()
    ApiResponse<ChatMessagesResponse> updateMenu(@RequestBody @Valid ChatMessagesRequest request, @RequestParam long id) {
        return ApiResponse.<ChatMessagesResponse>builder()
